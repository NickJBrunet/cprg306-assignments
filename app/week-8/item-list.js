...
Author: Nick Brunet
Description: Week 8 modified item list component from week 6
Modified: 2025-11-05
...
export default function ItemList({highLevelItems, onIngredientChange}) {
            ...
            <div className="flex justify-center">
                <ul className="grid grid-cols-4 text-center">
                    {items.map((item, index) => 
                        <Item key={index} index={index} props={item} onItemSelect={onIngredientChange}/>)
                    }
                </ul>
            </div>
        </div>
